// Code JS Version 1
// Start
console.log('Loading Code JS Framework/Library...');
...
const layout = {
    ...
    destroy : () => {
        ...
        console.log('The layout for this app has been destroyed/removed.');
    }
};
...
const addContentFirst = (target, $content) => {
    target.innerHTML = $content + target.innerHTML;
};

const addContentLast = (target, $content) => {
...
const restrictAll = (obj)=>{
    for (i = 0; i < obj.length; i++) {
        return obj[i].setAttribute('disabled', true);
    }
};
...
const unrestrictAll = (obj)=>{
    for (i = 0; i < obj.length; i++) {
        return obj[i].setAttribute('disabled', false);
    }
};
...
const animate = (obj, $time) => {
    ...
    function _auto_slide() {
        for (i = 0; i < obj.length; i++) {
            return obj[i].setAttribute('hidden', true);
        }
        ...
        obj[$Count - 1].setAttribute('hidden', false);
...
/* Visibility State */
const hide = (obj) => {
    return obj.setAttribute('hidden', true);
};
...
const unhide = (obj) => {
    return obj.setAttribute('hidden', false);
};

const unhideAll = (obj)=>{
    for (i = 0; i < obj.length; i++) {
        return obj[i].setAttribute('hidden', false);
    }
};
...
const loopSelect = ($group, $target, func) => {
    if ($target == "even") {
...
function switch_content($class, $id) {
    const cc = document.getElementsByClassName($class);
    for (_i = 0; _i < cc.length; _i++) {
        cc[_i].style.display = 'none';
    }
    document.getElementById($id).style.display = 'block'; $GET_PAGE_TITLE = document.querySelector('title'); $GET_PAGE_TITLE.innerHTML = $id + " - " + app.name;}
...
const app = {
    ...
    backgroundData : () => {
        ...
        addContentLast($('.CodeJS-Console h2'), `<br><span style="background:green; color: white;">${$msg}</span>`);

        // App Time
        addContentLast($('.CodeJS-Console'), '<section class="code_js_app_timer" style="padding: 1%; display: block; width: 100%;"></section>');
        $secs = -1;
        $mins = 0;
        $hrs = 0;
        $days = 0;
...
function _start() {
    ...
    function LOADFULLPACKAGE() {
        ...
        const app_Footnote = app.author + "<br> Code JS Developer <br> &copy; Copyright " + app.copyright + ".";

        if (document.querySelector('footnote')) {
            document.querySelector('footnote').innerHTML = app_Footnote;
        }
...
const loadContent = (func) => {
    ...
    const load_Content = setInterval(()=>{
        ...
        if ($('[role=content_loader]').value == $('[role=content_loader]').max) {
            func();
